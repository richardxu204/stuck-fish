...
class ChessBoard(tk.Tk):
    def __init__(self, size=100):
        ...
        self.geometry(
            "800x800"
            + "+"
            + str(int((super().winfo_screenwidth() - 800) / 3))
            + "+"
            + str(int((super().winfo_screenheight() - 800) / 4))
        )
    ...
    def bot_move(self):
        if self.player_turn == 0:
            ...
            self.chess.push(
                chess.Move.from_uci(self.stuckfish.engine.simple_selection("b"))
            )
    ...
    def left_click(self, event):
        ...
        if self.player_turn == 1:
            if self.selection == 0:
                self.overlay = [
                    [0 for x in range(self.rows)] for y in range(self.columns)
                ]
                ...
                if (
                    self.get_piece(
                        int(convert_col(self.piece_square[0]) - 1),
                        (8 - int(self.piece_square[1])),
                    )
                    == "P"
                    and int(clicked_square[1:]) == 8
                ):
                    self.chess_popup(self.canvas, "promotion")
                    self.chess.push(
                        chess.Move.from_uci(
                            self.piece_square + clicked_square + self.promotion_piece
                        )
                    )
                    self.promotion_piece = ""
                    self.fen = self.chess.fen()
                    ...
                    self.chess.push(
                        chess.Move.from_uci(self.piece_square + clicked_square)
                    )
                    self.fen = self.chess.fen()
                    # playsound('sounds/piecemove.wav')
                    if self.play_type == 1:
                    ...
                    clicked_square not in self.get_piece_moves(self.piece_square)
            ...
            for col in range(self.columns):
                ...
                if self.overlay[row][col] == 1:
                    ...
                    self.canvas.create_rectangle(
                        ...
                        y2,
                        outline="black",
                        fill=self.highlight_color2,
                        tags="empty",
                    )
                else:
                    self.canvas.create_rectangle(
                        x1, y1, x2, y2, outline="black", fill=color, tags="empty"
                    )
                color = self.color1 if color == self.color2 else self.color2
    ...
    def chess_popup(self, master, box_type):
        ...
        if box_type == "menu":
            self.start = OptionDialog(
                self.canvas,
                "Play Stuckfish",
                "Select your opponent!",
                ["Play Friend", "Play Stuckfish", "Quit"],
            )
        ...
        # popups for the different possible outcomes
            ...
            self.start = OptionDialog(
                self.canvas, "Game Over", "Checkmate!!!", ["Quit"]
            )
            if self.start.result == "Quit":
                master.destroy()
                quit()
            ...
            self.start = OptionDialog(
                self.canvas, "Game Over", "Stalemate!!!", ["Quit"]
            )
            if self.start.result == "Quit":
                master.destroy()
                quit()
            ...
            self.start = OptionDialog(
                self.canvas, "Game Over", "Draw: insufficient material!", ["Quit"]
            )
            if self.start.result == "Quit":
                master.destroy()
                quit()
            ...
            self.start = OptionDialog(
                self.canvas, "Game Over", "Draw: threefold repetition", ["Quit"]
            )
            if self.start.result == "Quit":
                master.destroy()
                quit()
            ...
            self.start = OptionDialog(
                self.canvas, "Check", "Your king is in check", ["Ok sorry I'm blind"]
            )
            if self.start.result == "Quit":
                master.destroy()
                quit()
