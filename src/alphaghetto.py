...
import torch
import sqlite3
from src.stuckfishsupport import *
...
class AlphaGhetto(object):
    ...
    def calculate_total_material(self, perspective, capture_value=0):
        material_score = 0
        ...
        for piece_rank in range(len(self.position)):
            for piece_file in range(len(self.position[piece_rank])):
                ...
                if perspective == "b":
                    if self.get_piece(piece_file, piece_rank) in [
                        "q",
                        "r",
                        "b",
                        "n",
                        ...
                        material_score = material_score + self.calculate_piece_value(
                            piece_file, piece_rank, perspective, capture_value
                        )
                elif perspective == "w":
                    if self.get_piece(piece_file, piece_rank) in [
                        "Q",
    ...
    def calculate_piece_value(self, piece_x, piece_y, perspective, capture_value=0):
        ...
        # if capture_value > 0:
        # piece_value = piece_value + self.calculate_capture_prospects(piece_x, piece_y, perspective, capture_value)
        return piece_value
    ...
    def calculate_piece_bonus(self, piece_x, piece_y):
        ...
        if self.get_piece(piece_x, piece_y) == "P":
            ...
            for attacker in self.get_attacker_locations(
                self.get_square_name(piece_x, piece_y).capitalize(), "w"
            ):
            ...
            for attacker in self.get_attacker_locations(
                self.get_square_name(piece_x, piece_y).capitalize(), "b"
            ):
                defender_points = defender_points + int(
                    self.piece_values.get(self.get_piece_at(attacker))
                )
    ...
    def calculate_capture_prospects(self, piece_x, piece_y, perspective, value):
        bonus = 0
        attacker_points = 0
        defender_points = 0
        # attacked_piece = self.get_square_name(piece_x, piece_y)
        if perspective == "b":
            if (
                len(
                    self.get_attacker_locations(
                        self.get_square_name(piece_x, piece_y).capitalize(), "w"
                    )
                )
                > 0
            ):
                for attack in self.get_attacker_locations(
                    self.get_square_name(piece_x, piece_y).capitalize(), "w"
                ):
                    attacker_points = (
                        attacker_points + self.piece_values[self.get_piece_at(attack)]
                    )
            if (
                len(
                    self.get_attacker_locations(
                ...
                for defend in self.get_attacker_locations(
                    self.get_square_name(piece_x, piece_y).capitalize(), "b"
                ):
                    defender_points = (
                        defender_points + self.piece_values[self.get_piece_at(defend)]
                    )
        elif perspective == "w":
            if (
                len(
        ...
        for move in selected_moves:
            ...
            if len(move[2:]) == 2:
                if self.get_piece_at(move[2:]) != 0:
                    considered_moves[move] = self.calculate_total_material(
                        perspective, self.piece_values[self.get_piece_at(move[2:])]
                    )
            ...
            if len(move[2:]) == 2 and self.get_piece_at(move[2:]) != 0:
                considered_moves[move] = considered_moves[
                    move
                ] + self.calculate_capture_immediate(
                    self.get_piece_at(move[:2]), self.get_piece_at(move[2:])
                )
    ...
    def get_attacker_locations(self, square_name, perspective):
        ...
        if perspective == "b":
            for square in self.chess.SQUARES:
                if square in self.board.attackers(
                    chess.BLACK, getattr(chess, self.square_name)
                ):
                    attackers.append(chess.square_name(square))
        if perspective == "w":
            for square in self.chess.SQUARES:
                if square in self.board.attackers(
                    chess.WHITE, getattr(chess, self.square_name)
                ):
                    attackers.append(chess.square_name(square))
        return attackers
    ...
    def get_attacking_locations(self, square_name, perspective):
        ...
        if perspective == "b":
            for square in self.chess.SQUARES:
                if square in self.board.attacks(
                    chess.BLACK, getattr(chess, self.square_name)
                ):
